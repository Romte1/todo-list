...
function loadTasksList(projectName) {
  ...
  for (let i = 0; i < currentProjectTasks.length; i++) {
    ...
    taskDateP.textContent = format(
      parseISO(currentProjectTasks[i].date),
      "MM-dd-yy",
    );
...
function loadTasksByDate(dateSection) {
  ...
  const relevantTasks = tasks.filter(
    (obj) =>
      differenceInDays(normalizeDate(dateParse(obj.date)), today) < dateLimit &&
      differenceInDays(normalizeDate(dateParse(obj.date)), today) >= 0,
  );
...
function loadCompletedTasks() {
  ...
  for (let i = 0; i < completedTasks.length; i++) {
    ...
    taskDateP.textContent = format(
      parseISO(completedTasks[i].date),
      "MM-dd-yy",
    );
