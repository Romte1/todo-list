  ...
  addEventListener("click", ({ target }) => {
    if (target === newProjectBtn && newProjectIpt.value !== "") {
      for (let i = 0; i < Projects.length; i++) {
        if (
          Projects[i].name.toLowerCase() === newProjectIpt.value.toLowerCase()
        ) {
...
function editProject(name) {
  ...
  editProjectConfirmButton.addEventListener("click", () => {
    ...
    for (let i = 0; i < Projects.length; i++) {
      if (
        Projects[i].name.toLowerCase() === newName.toLowerCase() &&
        renameProjectIpt.value.toLowerCase() !== name.toLowerCase()
      ) {
        editProjectMsg.setAttribute("class", "error");
        editProjectMsg.textContent =
          "You cannot rename a project to an existing project's name";
        //console.log(Projects.length);
        return;
      }
    }
